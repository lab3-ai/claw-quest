...
import { isEscrowConfigured, escrowConfig } from './escrow.config';
...
export async function escrowRoutes(server: FastifyInstance) {
    ...
    server.post(
        '/distribute/:questId',
        {
            schema: {
                tags: ['Escrow'],
                summary: 'Trigger on-chain distribution (operator only)',
                security: [{ bearerAuth: [] }],
                params: z.object({ questId: z.string().uuid() }),
                body: z.object({
            ...
            onRequest: [server.authenticate, requireAdmin],
        },
        async (request, reply) => {
            if (!isEscrowConfigured()) {
                return reply.status(503).send({ message: 'Escrow not configured' } as any);
            ...
            // Check quest exists and is funded
    ...
    server.post(
        '/refund/:questId',
        {
            schema: {
                tags: ['Escrow'],
                summary: 'Trigger on-chain refund (admin only)',
                security: [{ bearerAuth: [] }],
                params: z.object({ questId: z.string().uuid() }),
                body: z.object({
                    chainId: z.number().optional(),
