...
import { CheckFill, User3Fill, AiFill, AddLine } from "@mingcute/react";
        ...
        <div className="pl-10 pb-4">
          <div className="p-4 sm:p-6 border border-border-2 rounded bg-bg-1">
            <div className="space-y-4 mb-6">
              ...
              {/* ── Human Tasks ── */}
              <div className="mb-6 border border-border-2 rounded-lg p-4 bg-bg-base">
                <div className="flex items-center gap-2 text-sm font-semibold mb-3">
                  <User3Fill size={16} className="text-(--human-fg)" /> Human
                  Tasks
                  <span className="text-xs font-semibold px-1.5 py-0.5 rounded uppercase tracking-wider bg-(--human-bg) text-(--human-fg)">
                    Social
                  </span>
                  ...
                  <div className="flex border-b border-border-2">
                    {Object.keys(PLATFORM_ACTIONS).map((p) => (
                      <button
                        key={p}
                        className={cn(
                          "flex items-center gap-1.5 px-4 py-2 text-sm font-medium cursor-pointer transition-colors border-b-2 -mb-px",
                          activePlatform === p
                            ? "border-primary text-fg-1"
                            : "border-transparent text-fg-3 hover:text-fg-1 hover:border-border",
                        )}
                        onClick={() =>
                          onSetActivePlatform(activePlatform === p ? null : p)
                        }
                      >
                        <PlatformIcon
                      ...
                      <div className="flex flex-col gap-2 overflow-hidden">
                        {PLATFORM_ACTIONS[activePlatform]
                          ...
                          .map((action) => (
                            <div
                              key={action.type}
                              className="flex items-center justify-between px-3 py-2 bg-bg-1 rounded border border-border-2 text-sm cursor-pointer transition-colors hover:bg-bg-2"
                              onClick={() =>
                ...
                {humanTasks.length === 0 ? (
                  <div className="text-xs text-fg-3 py-2">
                    No tasks added yet. Use the template picker above.
                  </div>
                ) : (
                  <>
                    <div className="text-xs font-semibold text-fg-1 mb-2">
                      Added Tasks
                    </div>
                    {humanTasks.map((task, i) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="font-semibold text-fg-1 flex-1">
                            {task.action}
                          </span>
                          {task.chips.length > 0 && (
                            <span className="inline-flex items-center justify-center min-w-4 h-4 rounded-full text-xs font-semibold bg-accent-light0 text-white px-1">
                              {task.chips.length}
                            </span>
                          )}
                          <span className="text-xs font-semibold px-1.5 py-0.5 rounded bg-(--social-bg) text-(--social-fg) uppercase">
                            {task.platform}
                          </span>
                          <button
                        ...
                        {expandedTask !== i && task.chips.length > 0 && (
                          <div className="flex flex-wrap gap-1 px-3 py-1.5 pb-2 text-xs border-t border-dashed border-border-2">
                            ...
                            {task.chips.length > 4 && (
                              <span className="font-mono text-xs text-fg-3 bg-bg-3 px-1.5 py-px rounded">
                                +{task.chips.length - 4} more
                              </span>
                            )}
                          </div>
                        )}
              ...
              {/* ── Agent Tasks ── */}
              <div className="mb-6 border border-border-2 rounded-lg p-4">
                <div className="flex items-center gap-2 text-sm font-semibold mb-3">
                  <AiFill size={16} className="text-(--agent-fg)" /> Agent Tasks
                  <span className="text-xs font-semibold px-1.5 py-0.5 rounded uppercase tracking-wider bg-(--agent-bg) text-(--agent-fg)">
                    Skill
                  </span>
                  <span className="font-normal text-xs text-fg-3 ml-auto">
                ...
                <div className="bg-bg-3/50 border border-border-2 rounded px-3 py-2 text-xs text-fg-3 leading-relaxed mb-3">
                  ...
                  installed + each required skill below. CQ checks capabilities
                  before submission.
                </div>

                <div className="text-xs font-semibold text-fg-1 mt-3 mb-2">
                  Required Skills
                </div>
                {requiredSkills.length === 0 ? (
                  <div className="text-xs text-fg-3 py-1 pb-2">
                              ...
                              <div className="flex-1 min-w-0 overflow-hidden">
                                <div className="text-xs font-semibold text-primary flex items-center gap-1.5 truncate">
                                  {s.ownerImage ? (
                                    <img
                                      src={s.ownerImage}
                                      alt={s.ownerDisplayName ?? s.ownerHandle}
